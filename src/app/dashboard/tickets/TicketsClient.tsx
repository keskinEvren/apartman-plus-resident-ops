...
export function TicketsClient() {
  ...
  // 2. Fetch Tickets
  const { data: myTickets = [], isLoading: loadingMyTickets } = trpc.ticket.listMyTickets.useQuery(undefined, { enabled: !isStaff && !!activeSiteId });
  const { data: staffTickets = [], isLoading: loadingStaffTickets } = trpc.ticket.listStaffTickets.useQuery(undefined, { enabled: isStaff && !!activeSiteId });
  ...
  const assignMutation = trpc.ticket.assignTicket.useMutation({
    onSuccess: () => {
      showToast("success", "Talep üzerinize atandı ve İŞLEMDE olarak güncellendi!");
      utils.ticket.listStaffTickets.invalidate();
      if (selectedTicket) setSelectedTicket((prev: any) => ({ ...prev, status: "IN_PROGRESS" }));
    },
  ...
  const statusMutation = trpc.ticket.updateTicketStatus.useMutation({
    onSuccess: (data) => {
      ...
      utils.ticket.listStaffTickets.invalidate();
      utils.ticket.listMyTickets.invalidate();
      if (selectedTicket) setSelectedTicket((prev: any) => ({ ...prev, status: data.status }));
    },
  ...
  const pendingCount = activeTickets.filter((t) => t.status === "OPEN" || t.status === "IN_PROGRESS").length;
  ...
  return (
    <div className="space-y-8 animate-fade-in">
      <TicketsHeader isStaff={isStaff} blockName={currentMembership?.blockId ? "A Blok" : null} onOpenAdd={() => setIsAddOpen(true)} />
      <TicketStats pendingCount={pendingCount} />
      <section className="space-y-4">
        <h2 className="text-sm font-semibold text-muted-foreground uppercase tracking-wider">{isStaff ? "Site Destek Talepleri" : "Talep Geçmişim"}</h2>
        {isStaff ? (
          <StaffTicketList
            ...
            onSelect={(ticket) => { setSelectedTicket(ticket); setIsDetailsOpen(true); }}
      ...
      <CreateTicketModal isOpen={isAddOpen} onClose={() => setIsAddOpen(false)} onCreate={(c, t, d) => createMutation.mutate({ category: c, title: t, description: d })} isPending={createMutation.isPending} />
      <TicketDetailModal
        ...
        onUpdateStatus={(status) => statusMutation.mutate({ ticketId: selectedTicket.id, status })}
        statusLabels={statusLabels}
        statusColors={statusColors}
        categoryLabels={categoryLabels}
      />
